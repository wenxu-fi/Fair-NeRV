function [t,cluster_stable_f1average_arr]=f1average_parallel_fnerv_local(data_title, sensstr,initial_Y,sigma2,P,nclasses,classes,nclusters,clusters,K,intra_start,intra_end,inter_end,beta_start,beta_end,gamma_start,gamma_end,contrast_coeffi_start,contrast_coeffi_end,niters,learningrate,exaggeration,n,rng_seed,output_folder)
    ...
    I4=randperm(n);
    I5=randperm(n);
    %tradeoff inter parameter is greater than tradeoff intra
    ...
    parfor k=1:n
        ...
        optimized_Y=optimize_fnerv(P,nclasses,classes,initial_Y,sigma2,tradeoff_intra,tradeoff_inter,beta,gamma,niters,learningrate,exaggeration,coeffi);

        filename=fullfile('../results',output_folder,'fnerv','optresults',sprintf(data_title+sensstr+"fnerv results tradeoff_intra(%.3f) tradeoff_inter(%.3f) beta(%.3f) gamma(%.3f) contrast_coeffi(%.3f).mat", tradeoff_intra,tradeoff_inter, beta,gamma,coeffi));
        parsave(filename,optimized_Y); %save optimized data
          
        %compute f1 scores of knn prediction of sensitive attribute and high
    ...
    %save to file
    filename=fullfile('../results',output_folder,'fnerv','summary',sprintf(data_title+sensstr+"fnerv cluster stable f1average outputs.mat"));
